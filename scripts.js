...
async function getTopSimps(videoId) {
  try {
    const response = await fetch(
      `http://localhost:3000/topSimps?video_id=${videoId}`
    );
    const data = await response.json();
    console.log("TOP SIMPS DATA:", data);
    return data.data.top_chat_users;
  } catch (error) {
    console.error("Error:", error);
    throw error;
  }
...
async function updateTopSimps() {
  ...
  // Use Promise.all to wait for all promises to resolve
  const topSimpsPromises = videoIds.map((id) => getTopSimps(id));
  videoTopSuperChatUsers = await Promise.all(topSimpsPromises);
}
