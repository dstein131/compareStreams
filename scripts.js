...
async function updateSuperChatTotals() {
  const videoIds = [
  ...
  const superchatTotalsPromises = activeVideoIds.map((id) =>
    getSuperchatTotal(id)
  );
  videoSuperChatTotals = await Promise.all(superchatTotalsPromises);
}
...
document.addEventListener("DOMContentLoaded", function () {
  document
    ...
    .addEventListener("submit", async function (event) {
      ...
      await updateSuperChatTotals(); // Update super chat totals
...
function compareLiveStreams(videoIds) {
  ...
  const fetchExchangeRate = async (currency) => {
    ...
    try {
      const res = await fetch(
        `https://v6.exchangerate-api.com/v6/d653f6d01951d04f8ef1ca0e/latest/${currency}`
      );
      const data = await res.json();
      return (exchangeRates[currency] =
        data.result === "error" ? 1 : data.conversion_rates.USD);
    } catch (error) {
      return 1;
    }
  };
  ...
  videoIds.forEach((videoId, index) => {
    ...
    if (!document.getElementById(`iframe-${index + 1}`)) {
      videoIframe.innerHTML = `<iframe id="iframe-${
        index + 1
      }" src="https://www.youtube.com/embed/${videoId}?autoplay=1&mute=1" frameborder="0" allowfullscreen></iframe>`;
    }
...
function prepareDataAndSend() {
  ...
  for (let i = 1; i <= 4; i++) {
    ...
    if (Object.values(elements).every(Boolean) && topSuperchatUsers) {
      const usersArray = topSuperchatUsers
        .split("\n")
...
async function getSuperchatTotal(videoId) {
  try {
    const response = await fetch(
      `http://localhost:3000/superchatData?video_id=${videoId}`
    );
    const data = await response.json();
    console.log("SUPERCHAT DATA:", data);
    return data.data;
  } catch (error) {
